...
- **Serverless State Sync:** Managing deck and turn logic without a central server required a **Host-based Authority** pattern where the player with the lowest `seat_index` acts as the host. All actions are logged to a `game_moves` table to ensure synchronized animations across all clients.
- **The Reaper & Heartbeat System:** To prevent game freezes from disconnects, I implemented a **Heartbeat** system (5s pulse) and "Reaper" logic to remove "ghost" players inactive for 60s. Administrative duties migrate seamlessly to the next available `seat_index` if a host leaves.
- **Animation & State Coordination:** Integrating complex rules with high-fidelity visuals required strict **State Locking**. Using a `processing` state and async delays ensures animations for "Lucky Draws" or "Sets" (Books) finish playing before database updates progress the turn.
- **Smart AI Strategy Engine:** Solo mode features bots with **Memory Modeling** to track opponent moves and a **Human Attention Bias** that forces bots to prioritize targeting the user to maintain engagement.
